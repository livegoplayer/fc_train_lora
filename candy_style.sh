...
# Train Model | 训练模式
model="db" #lora、db、sdxl_lora、sdxl_db、contralnet(unfinished)

# Train data path | 设置训练用模型、图片
pretrained_model="/home/ubuntu/stable-diffusion-webui/models/Stable-diffusion/Anything-ink.safetensors" # base model path | 底模路径
is_v2_model=0                             # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
v_parameterization=0 # parameterization | 参数化 v2 非512基础分辨率版本必须使用。
vae=""
train_data_dir="/home/ubuntu/train2"              # train dataset path | 训练数据集路径
reg_data_dir=""      # directory for regularization images | 正则化数据集路径，默认不使用正则化图像。
training_comment="this LoRA model credit from bug"	# training_comment | 训练介绍，可以写作者名或者使用触发关键词
...
network_dim=512   # network dim | 常用 4~128，不是越大越好
network_alpha=256 # network alpha | 常用与 network_dim 相同的值或者采用较小的值，如 network_dim的一半 防止下溢。默认值为 1，使用较小的 alpha 需要提升学习率。
...
adaptive_noise_scale="0.1" #adaptive noise scale | 自适应噪声偏移范围
...
keep_tokens=7  # keep heading N tokens when shuffling caption tokens | 在随机打乱 tokens 时，保留前 N 个不变。
...
lr_warmup_steps=500                   # warmup steps | 仅在 lr_scheduler 为 constant_with_warmup 时需要填写这个值
lr_restart_cycles=1                 # cosine_with_restarts restart cycles | 余弦退火重启次数，仅在 lr_scheduler 为 cosine_with_restarts 时起效。

min_snr_gamma=0 #最小信噪比伽马值，减少低step时loss值，让学习效果更好。推荐3-5，5对原模型几乎没有太多影响，3会改变最终结果。修改为0禁用。
weighted_captions=1 #权重打标，默认识别标签权重，语法同webui基础用法。例如(abc), [abc], (abc:1.23),但是不能再括号内加逗号，否则无法识别。
...
output_name="candy_style"           # output model name | 模型保存名称
